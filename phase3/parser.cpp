...
static void declarator(int spec)
{
    ...
    if (lookahead == '[') {
        match('[');
        int length = expectNum(NUM);
        match(']');
        declareArray(name,Type(ARRAY,spec,count,length));
    } else {
        declareVar(name,Type(SCALAR,spec,count));
    }
}
...
static void primaryExpression()
{
    if (lookahead == '(') {
        ...
        if (lookahead == '(') {
            ...
            match(')');
            useSymbol(name,Type(FUNCTION,INT));
        } else {
            useSymbol(name,Type(SCALAR));
        }
    } else
        error();
}
...
static void parameter(Parameters* param)
{
    int spec = specifier();
    int ind = pointers();
    string name = expect(ID);
...
static void globalDeclarator(int spec)
{
    ...
    if (lookahead == '[') {
        ...
        parameters(param);
        match(')');
        declareFunc(name,Type(FUNCTION,spec,count,0,param));
    } else {
        declareVar(name,Type(SCALAR,spec,count));
    }
}
...
static void topLevelDeclaration()
{
    ...
    if (lookahead == '[') {
        ...
        int length = expectNum(NUM);
        match(']');
        declareArray(name,Type(ARRAY,spec,ind,length));
        ...
        if (lookahead == '{') {
            defineFunc(name,Type(FUNCTION,spec,ind,0,param));
            match('{');
            declarations();
            statements();
            closeScope();
            ...
            closeScope();
            declareFunc(name,Type(FUNCTION,spec,ind,0,param));
            remainingDeclarators(spec);
        }
